            ...
            <div className="bg-white dark:bg-neutral-800 transition-colors p-6 md:p-12 rounded-[32px] border border-gray-100 dark:border-neutral-700 shadow-[0_8px_30px_rgb(0,0,0,0.04)] mx-auto w-full max-w-[900px]">
                ...
                <div className="relative max-w-2xl">
                   ...
                   <div className="space-y-4">
                      {detail.sections?.prize?.items?.map((item, i) => (
                        <div key={i} className="bg-white dark:bg-neutral-800 transition-colors shadow-[0_4px_20px_rgb(0,0,0,0.03)] p-6 rounded-[20px] flex justify-between items-center border border-gray-100 dark:border-neutral-700 hover:border-cta hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all">
                         <div className="flex items-center gap-4">
                            <span className="text-3xl">{item.place === '1st' ? '🥇' : item.place === '2nd' ? '🥈' : item.place === '3rd' ? '🥉' : '🏅'}</span>
                            <strong className="text-[18px] text-primary dark:text-white transition-colors uppercase tracking-wider">{item.place} PLACE</strong>
                         </div>
              ...
              {tab.id === 'submit' && (
                <div className="relative w-full">
                   ...
                   <div className="bg-blue-50 dark:bg-cta/10 transition-colors border border-blue-100 p-6 md:p-8 rounded-[24px] mb-10 w-full max-w-3xl">
                    ...
                    {hackathon?.status === 'ended' ? (
                      <div className="bg-gray-50 dark:bg-neutral-800/50 transition-colors border border-gray-200 dark:border-neutral-700 p-10 rounded-[32px] text-center w-full max-w-3xl">
                        <div className="w-16 h-16 bg-gray-200 dark:bg-neutral-700 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl transition-colors">⏰</div>
                      ...
                      <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl bg-gray-50/50 p-6 md:p-8 rounded-[32px] border border-gray-200 dark:border-neutral-700 transition-colors w-full relative overflow-hidden">
                        {!currentUser && (
                          ...
                          {submissionType === 'team' && !submitTeamName && (
                            <div className="mb-6 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 p-4 rounded-[16px] flex items-start gap-3 transition-colors">
                              ...
                              <div>
                                ...
                                <p className="text-[13px] text-red-600 dark:text-red-300 font-medium mt-1 transition-colors">이 해커톤에 참여 중인 팀이 없습니다. 팀 제출을 원하시면 먼저 [참여 팀 현황] 탭에서 팀에 가입하거나 생성해주세요.</p>
                              </div>
                            </div>
                          )}
                     ...
                     <div className="mt-12 w-full max-w-3xl">
                       ...
                       <div className="space-y-4">
                          {recentSubmissions.map((s) => (
                             <div key={s.id} className="bg-gray-50 dark:bg-neutral-800/50 transition-colors p-6 rounded-[20px] border border-gray-100 dark:border-neutral-700 flex flex-col gap-2 relative overflow-hidden group">
                               <div className="absolute top-0 left-0 bottom-0 w-1.5 bg-gray-200 dark:bg-neutral-600 group-hover:bg-cta transition-colors"></div>
                              ...
                              <tr key={entry.teamName} className="hover:bg-gray-50 dark:hover:bg-neutral-700 transition-colors /50">
                               <td className="py-4 px-6">
                                 <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full font-black text-[13px] ${
                                   entry.rank === 1 ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 transition-colors' : 
                                   entry.rank === 2 ? 'bg-gray-100 dark:bg-neutral-700 transition-colors text-gray-500 dark:text-gray-400' : 
